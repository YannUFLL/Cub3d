...
/*   Updated: 2022/07/19 14:59:33ydumaine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_draw_line(t_data *data, t_ray *ray, t_flo *flo, int x)
{
	...
	while(ray->drawend < ray->resolution_y + 1)
	{
		...
		if (width * flo->floortexty + flo->floortextx >= 261840 || width * flo->floortexty + flo->floortextx <= 0)
		{
			printf("width : %d\n",width);
			printf("texx : %d\n",flo->floortextx);
			printf("text : %d\n",flo->floortexty);
			printf("result: %d \n", width * flo->floortexty + flo->floortextx);
		}
		my_mlx_pixel_put(data, x, ray->drawend,
			text1[width * flo->floortexty + flo->floortextx]);
		/*my_mlx_pixel_put(data, x, ray->resolution_y 
			- ray->drawend, text2[width * flo->floortexty + flo->floortextx]);*/
		ray->drawend++;
	}
}
